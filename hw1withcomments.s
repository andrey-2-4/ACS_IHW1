...
min:
	...
	cmp	eax, DWORD PTR -8[rbp] # сравниваем eax и (-8 на стеке)
	jge	.L2 # если >=, то переходим на .L2
	mov	eax, DWORD PTR -4[rbp] # eax = (-4 на стеке)
	jmp	.L3 # переходим на метку .L3
.L2:
	mov	eax, DWORD PTR -8[rbp] # eax = (-8 на стеке)
.L3:
...
main:
	...
	mov	DWORD PTR -4[rbp], 0 # (-4 на стеке) = a = 0
...
.L9: 	# тело цикла (заполняем массив, выводим, складываем элементы)
	...
	add	DWORD PTR -4[rbp], eax # прибавляем eax (элемент) к (-4 на стеке) = a
	mov	eax, DWORD PTR -8[rbp] # eax = (-8 на стеке) = i
	cdqe # расширяем eax до rax
	# вычисляем положение элемента №i
	lea	rdx, 0[0+rax*4]
...
.L8: 	
	...
	mov	DWORD PTR -4[rbp], eax # записываем результат деления в a
